...
void UState_GBEC_Attack::Init(class UStateMng_GC* pMng)
{
	Super::Init(pMng);

}

void UState_GBEC_Attack::Enter()
{
	Super::Enter();
	m_fReadyTime = 0.0f;
	m_bMotion = false;
...
void UState_GBEC_Attack::Update(float fDeltaTime)
{
	...
	if (m_bAttacked == false)
	{
		...
		{
			m_bAttacked = true;
			GetRootChar()->GetAnimationMng()->PlayAnimationMontage(TEXT("AttackMtg"), 1.0f, TEXT("Attack"));
		}
	}
...
void UState_GBEC_Attack::StateMessage(FString StateMessage)
{
...
void UState_GBEC_Attack::Attack()
{
...
void UState_GBEC_Attack::AttackEnd()
{
...
void UState_GBEC_Attack::Tick_Attack(float fDeltaTime)
{
